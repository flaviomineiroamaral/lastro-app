...
DROP POLICY "Membros veem membros da mesma org" ON "public"."organization_members";

ALTER TABLE "public"."regras_categorizacao"
  DROP CONSTRAINT "regras_categorizacao_tipo_leitura_check";

ALTER TABLE "public"."transacoes_staging"
  DROP CONSTRAINT "transacoes_staging_status_importacao_check";

ALTER TABLE "public"."transacoes_staging"
  DROP CONSTRAINT "transacoes_staging_tipo_operacao_check";

DROP FUNCTION "public"."fn_obter_contrato_b2b_by_serial"(uuid);
...
CREATE OR REPLACE FUNCTION public.fn_ativar_assinatura_b2c (
  p_cpf text
)
  RETURNS json
  LANGUAGE plpgsql
...
DECLARE
    v_id uuid;
BEGIN
    -- Procura a assinatura pelo CPF e atualiza o status numa única transação
    UPDATE public.assinaturas_b2c
    SET 
        status_assinatura = 'ativa',
        ultima_cobranca = now()
    WHERE cpf = p_cpf AND status_assinatura = 'pendente'
    RETURNING id INTO v_id;

    IF FOUND THEN
        RETURN json_build_object('sucesso', true, 'mensagem', 'Assinatura ativada com sucesso.', 'id', v_id);
    ELSE
        RETURN json_build_object('sucesso', false, 'mensagem', 'CPF não encontrado ou já ativado.');
    END IF;
END;
$function$;
...
CREATE OR REPLACE FUNCTION public.fn_vincular_usuario_ao_serial (
...
BEGIN
    -- 1. Tenta vincular na tabela B2B
    UPDATE public.contratos_b2b
    SET 
        serial_utilizado = true,
        user_id = p_user_id
    WHERE serial_ativacao = p_serial_ativacao AND serial_utilizado = false;

    IF FOUND THEN
        RETURN json_build_object('sucesso', true, 'tabela', 'contratos_b2b');
    END IF;

    -- 2. Tenta vincular na tabela B2C
    UPDATE public.assinaturas_b2c
    SET 
        serial_utilizado = true,
        user_id = p_user_id
    ...
    IF FOUND THEN
        RETURN json_build_object('sucesso', true, 'tabela', 'assinaturas_b2c');
    END IF;

    -- 3. Se não achou em lado nenhum ou já estava queimado
    RETURN json_build_object('sucesso', false, 'mensagem', 'Serial inválido ou já utilizado.');
END;
$function$;
...
ALTER TABLE "public"."regras_categorizacao"
  ADD CONSTRAINT "regras_categorizacao_tipo_leitura_check"
...
ALTER TABLE "public"."transacoes_staging"
  ADD CONSTRAINT "transacoes_staging_status_importacao_check"
...
ALTER TABLE "public"."transacoes_staging"
  ADD CONSTRAINT "transacoes_staging_tipo_operacao_check"
...
CREATE POLICY "Atualização B2C: Utilizadores atualizam apenas a própria ass" ON "public"."assinaturas_b2c"
  FOR UPDATE
  TO "authenticated"
  USING ((auth.uid() = user_id))
  WITH CHECK ((auth.uid() = user_id));

CREATE POLICY "Leitura B2C: Utilizadores veem apenas a própria assinatura" ON "public"."assinaturas_b2c"
  FOR SELECT
  TO "authenticated"
  USING ((auth.uid() = user_id));

CREATE POLICY "Leitura de Vinculo Pessoal" ON "public"."organization_members"
  FOR SELECT
  TO "authenticated"
  USING ((profile_id = auth.uid()));

GRANT EXECUTE ON FUNCTION "public"."fn_ativar_assinatura_b2c"(text) TO PUBLIC, "anon", "authenticated", "postgres", "service_role";

GRANT EXECUTE ON FUNCTION "public"."fn_vincular_usuario_ao_serial"(uuid, uuid) TO PUBLIC, "anon", "authenticated", "postgres", "service_role";
